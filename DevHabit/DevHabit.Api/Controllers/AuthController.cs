    ...
    IOptions<JwtAuthOptions> options) : ControllerBase
{
    ...
    public async Task<ActionResult<AccessTokensDto>> Register(RegisterUserDto registerUserDto)
    {
        ...
        IdentityResult addToRoleAsync = await userManager.AddToRoleAsync(identityUser, Roles.Member);
        ...
        if (!addToRoleAsync.Succeeded)
        {
            ...
            {
                {
                    "errors", addToRoleAsync.Errors.ToDictionary(e => e.Code, e => e.Description)
                }
            };
